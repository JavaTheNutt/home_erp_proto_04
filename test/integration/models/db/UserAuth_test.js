...
describe('user auth', function () {
	...
	describe('creation', function () {
		...
		before(async function(){
			'use strict';
			const tmpGroup = new UserGroup({name: 'test', users:[{email: 'test@test.com', firstName: 'test', surname: 'test'}]});
		...
		it('should fail when an incorrect group id is present', async function () {
			const authProvider = new AuthProvider({name: 'firebase', identifier: 'test'});
			const userAuth = new UserAuth({email: 'test@test.com', user: ObjectId().toString(), group: ObjectId().toString(), authProviders:[authProvider], roles: ['group_admin']});
			try {
		...
		it('should successfully create an auth object', async function(){
			'use strict';
			const authProvider = new AuthProvider({name: 'firebase', identifier: 'test'});
			const userAuth = new UserAuth({email: 'test@test.com', user: userId, group: groupId, authProviders:[authProvider], roles: ['group_admin']});
			const result = await userAuth.save();
